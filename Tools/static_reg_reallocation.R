...
#'
#' @author Juli?n D?az-Acosta, 06/03/2025.
#' @export

static_reg_reallocation<-function(data, y, x, fix_eff, weight_var, disag_var, k, reg_name, mfx_cond=T){
  ...
  for (fe in fix_eff) {
    ...
    for(variable in disag_var){
      ...
      if(mfx_cond){
        ...
        # # --- Extract baseline and contrast levels for the contrast variable ---
        # contrast_baseline <- levels(data[[contrast_var]])[1]
        # contrast_levels <- levels(data[[contrast_var]])[-1]
        # contrasts <- c(contrast_baseline, contrast_levels)
        # 
        # # --- Tidy up regression coefficients ---
        # coef_table <- broom::tidy(regression_all)
        # 
        # # --- Extract and format coefficients for the baseline level ---
        # baseline_coefs <- coef_table %>%
        #   filter(!str_detect(term, paste(contrast_levels, collapse = "|"))) %>%
        #   mutate(term = str_replace_all(term, ":+", "")) %>%
        #   mutate(!!contrast_baseline := add_stars(estimate, p.value),
        #          !!paste0(contrast_baseline, "_se") := paste0("(", round(std.error, 3), ")")) %>%
        #   select(term, all_of(c(contrast_baseline, paste0(contrast_baseline, "_se")))) %>%
        #   data.table::setDT()
        # 
        # # --- Compute marginal effects for all contrast groups ---
        # mfx <- slopes(
        #   regression_all,
        #   variables = unique(unlist(strsplit(x, "\\*"))),
        #   by = c(contrast_var, "patent_window", "tm_window")
        # ) %>%
        #   select(term, estimate, std.error, p.value, all_of(c(contrast_var, "patent_window", "tm_window"))) %>%
        #   tidyr::pivot_wider(names_from = all_of(contrast_var), values_from = c(estimate, std.error, p.value)) %>%
        #   data.table::setDT()
        # 
        # # --- Create readable interaction labels ---
        # mfx[, term := paste0(term,
        #                      ifelse(tm_window == 1, "tm_window", ""),
        #                      ifelse(patent_window == 1, "patent_window", ""))]
        # 
        # # --- Remove rows where contrast_var is mistakenly in the term label ---
        # mfx <- mfx[!grepl(contrast_var, term)]
        # 
        # # --- Loop over contrast levels to compute difference from baseline ---
        # for (lvl in contrast_levels) {
        #   
        #   # Extract rows related to this contrast level
        #   contrast_label <- paste0(contrast_var, lvl)
        #   contrast_coefs <- coef_table %>%
        #     filter(str_detect(term, contrast_label)) %>%
        #     mutate(
        #       term = case_when(
        #         term == contrast_label ~ "(Intercept)",
        #         TRUE ~ gsub(paste0("(^|:?)", contrast_label, "(:?|$)"), "\\1", term)
        #       ),
        #       term = str_replace_all(term, ":+", "")
        #     ) %>%
        #     mutate(
        #       !!paste0("difference_", lvl) := add_stars(estimate, p.value),
        #       !!paste0("difference_", lvl, "_se") := paste0("(", round(std.error, 3), ")")
        #     ) %>%
        #     select(term, starts_with("difference_")) %>%
        #     data.table::setDT()
        #   
        #   # Join contrast-level differences into main coefficient table
        #   baseline_coefs <- left_join(
        #     baseline_coefs,
        #     contrast_coefs,
        #     by = "term"
        #   )
