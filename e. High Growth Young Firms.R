...
firm_data <- merge(firm_data, patent_tm_data, by=c("firmid", "year"), all.x = T)
firm_data <- merge(firm_data, ipcr_cumulative, by=c("firmid", "year"), all.x=T)

# Create dummy indicators for patenting, trademarking and expanding patenting scope
firm_data[, patent:=ifelse(num_patent<=0 | is.na(num_patent), 0, 1)]
firm_data[, tm:=ifelse(num_tm<=0 | is.na(num_tm), 0, 1)]
firm_data[, ipcr_creat:=ifelse(ipcr_creat<=0 | is.na(ipcr_creat), 0, 1)]

firm_data<-window_var_cretor(firm_data, "firmid", "year", "patent", 2, 2, "patent_window", na_rm = T)
firm_data<-window_var_cretor(firm_data, "firmid", "year", "tm", 2, 2, "tm_window", na_rm = T)
firm_data<-window_var_cretor(firm_data, "firmid", "year", "ipcr_creat", 2, 2, "ipcr_creat_window", na_rm = T)

firm_data<-firm_data[abs(empl_growth)<2 & abs(nq_growth)<2]
firm_data[, log_firm_age:=log(firm_age)]
firm_data[, log_empl_bar:=log(empl_bar)]

saveRDS(firm_data, "ip_growth.RDS")

growth_vars<-c("empl", "nq", "tfp")
inno_vars<-c("patent_window", "tm_window", "ipcr_creat_window")
fix_eff<-"NACE_BR"
categories<-c("superstar", "superstar_cr4", "superstar_tfp_90", "superstar_tfp_99")
results<-data.table()

for(category in categories){
  for(growth_var in growth_vars){
    for(inno_var in inno_vars){
      for(yr in unique(firm_data$year)){
        for(filter in unique(firm_data[[category]])){
          print(paste0(yr, ", ", inno_var, ",", growth_var))
          
          firm_data_temp<-firm_data[year==yr & get(category)==filter]
          setDT(firm_data_temp)
          
          y<-paste0(growth_var, "_growth")
          x<-c(inno_var)
          weight_data<-firm_data_temp[[paste0(growth_var, "_bar")]]
          
          results_temp<-regression_reallocation_growth(data=firm_data_temp, 
                                                       y=y, 
                                                       x=x,
                                                       fix_eff = fix_eff,
          ...
          setDT(results_temp)
          results_temp[, year:=yr]
          results<-rbind(results, results_temp, fill=T)
        }
      }
      
      results_temp<-results[factor==y & !is.na(get(inno_var))]
      name<-inno_var
      clow_name<-paste0("clow_", inno_var)
      chigh_name<-paste0("chigh_", inno_var)
      ...
      results_temp$filter<-as.factor(results_temp$filter)
      
      ggplot(results_temp, aes(x=year, y=.data[[name]], color=filter, fill=filter)) +
        ...
        scale_x_continuous(breaks=seq(min(results_temp$year), max(results_temp$year), by=2))+
        # coord_cartesian(ylim=y_lim)+
        theme_minimal() +
        theme(plot.title = element_text(hjust=0.5))+
        labs(title= tools::toTitleCase(paste0("Correlation of ", gsub("_", " ", y), " with ", gsub("_", " ", inno_var))),
             fill=category,
             color=category,
             size="5% Significance",
             x="Year",
             y=paste0("Coefficient"))
      ggsave(paste0(output_dir, growth_var, "_", inno_var, "_", category, ".png"), height=4, width=8)
...
threshold_young<-5
threshold_growth_vector_og<-c(0.3, 0.2, 0.1, 0.05)

# Define set of dependent vars, weights fixed effects and lag/lead structure for regressions
growth_vars<-c("empl", "nq", "tfp")
product_summary[, NACE_BR_year:=paste0(NACE_BR, "_", year)]
fix_eff<-c("NACE_BR + year")
fix_eff_add<-c("NACE_BR + year", "NACE_BR_year", "NACE_BR + year + firmid")
n_lags_bw<-4
n_lags_fw<-4
static<-T
dynamic<-F
...
for(subset in c("young", "mature")){
  ...
  if(subset=="young"){
    ...
    # xs<-list(c("net_product_creat", "net_product_destr", "tm", "patent"))
    xs<-list(c("net_product_creat_window*patent_window","net_product_creat_window*tm_window", "net_product_creat_window*ipcr_creat_window", "net_product_destr_window", "log_n_products"),
             c("net_product_creat_window*patent_window","net_product_creat_window*tm_window", "net_product_destr_window", "log_n_products"))
               # c("net_product_creat_window*total_patent_growth","net_product_creat_window*total_tm_growth", "net_product_destr_window"))
  }else{
    if(subset=="mature"){
      contrast_vars<-c("superstar", "superstar_cr4", "superstar_tfp_99", "superstar_tfp_90")
      contrast_var<-"superstar"
      product_summary$superstar<-factor(product_summary$superstar, levels=c(F, T))
      product_summary$superstar_cr4<-factor(product_summary$superstar_cr4, levels=c(F, T))
      product_summary$superstar_tfp_99<-factor(product_summary$superstar_tfp_99, levels=c(F, T))
      product_summary$superstar_tfp_90<-factor(product_summary$superstar_tfp_90, levels=c(F, T))
      
      # xs<-list(c("net_product_creat", "net_product_destr", "tm", "patent"))
      xs<-list(c("net_product_creat_window*patent_window*superstar","net_product_creat_window*tm_window*superstar", "net_product_creat_window*ipcr_creat_window*superstar", "net_product_destr_window", "log_emp_rnd"),
               c("net_product_creat_window*patent_window*superstar_cr4","net_product_creat_window*tm_window*superstar_cr4", "net_product_creat_window*ipcr_creat_window*superstar_cr4", "net_product_destr_window",  "log_emp_rnd"),
               c("net_product_creat_window*patent_window*superstar_tfp_99","net_product_creat_window*tm_window*superstar_tfp_99", "net_product_creat_window*ipcr_creat_window*superstar_tfp_99", "net_product_destr_window", "log_emp_rnd"),
               c("net_product_creat_window*patent_window*superstar_tfp_90","net_product_creat_window*tm_window*superstar_tfp_90", "net_product_creat_window*ipcr_creat_window*superstar_tfp_90", "net_product_destr_window", "log_emp_rnd"))
  ...
  for(growth_var in growth_vars){
    ...
    ys<-c(paste0(growth_var, "_growth"))
    ...
    for(y in ys){
      ...
      output_dir_growth_var<-paste0(output_dir_og, growth_var, "/", subset, "/", y, "/")
      if(!dir.exists(output_dir_growth_var)){
        output_dir_creator(output_dir_growth_var)
      }
      
      # Set loop thresholds based on stage
      # THIS IS NOT CLEAN, FIX IT LATER
      threshold_growth_vector <- if (subset == "young" & y=="top_growth") threshold_growth_vector_og else threshold_growth_vector_og[1]
      
      for(threshold_growth in threshold_growth_vector){
        
        output_dir<-paste0(output_dir_growth_var, threshold_growth, "/")
        ...
        for (digit in c("all")) {
          ...
          continuing_firms <- product_summary_temp[abs(empl_growth)!=2 & abs(nq_growth)!=2] #& abs(rev_growth)!=2]
          ...
          continuing_firms<-continuing_firms[, c("firmid", "year")]
          continuing_firmids<-unique(continuing_firms$firmid)
          product_summary_temp<-product_summary_temp[firmid %in% continuing_firmids]
          product_summary_temp<-merge(product_summary_temp, continuing_firms, by=c("firmid", "year"), all.y = T)
          
          
          make_summary_stats(product_summary_temp, 
                             c("firm_age", "empl_bar", "nq_bar", "empl_growth", "nq_growth", "number_of_products", "prod_creat", "prod_destr", "patent", "tm"),
          ...
          if(static){
            ...
            # For the all sample, create results playing with the set of fixed effects
            results_fix_eff_add<-list()
            for(fe in fix_eff_add){
              ...
              # Run the regression
              results_temp<-feols(formula, product_summary_temp, weights = product_summary_temp[[paste0(growth_var, "_bar")]])
              
              # Store results
              results_fix_eff_add[[length(results_fix_eff_add)+1]]<-results_temp
            ...
            title<-toTitleCase(paste0("Regressions of ", 
                                      ...
                                      if(y=="top_growth") paste0(" (top ", threshold_growth, ")"),
                                      " on product and innovation variables - ",
                                      subset, " sample - Different Fixed Effects"))
            
            # Export results
            modelsummary(
              results_fix_eff_add,
              output = paste0(output_dir, "regressions_", y, "_fix_eff_add.tex"), 
              label = paste0(growth_var, "_", subset, "_", threshold_growth, "_", "regressions_", y, "_fix_eff_add"),
              stars = TRUE, 
              gof_omit = "Std.Errors|R2 Within|R2 Within Adj.|AIC|BIC|Log.Lik.",
              title = title,
              table = "\\begin{table}[H]\n\\centering\n%s\n\\end{table}"
            )
            print((paste0("Exported fix_eff_add")))
            ...
            if(subset=="mature"){
              ...
              for(i in 1:nrow(mfx_regressions)){
                ...
                print(xtable(table_temp, 
                             caption=toTitleCase(paste0("Regressions of ", gsub("_", " ", y), " on product and innovation variables by ", gsub("_", " ", contrast_var))), 
