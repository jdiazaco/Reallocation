...
bysort naf2d_num: egen mean_q = mean(l_mu_tl_m_tr_FSQ)
bysort naf2d_num: egen mean_r = mean(l_mu_tl_m_tr_BFSR) 
gen wedge = l_mu_tl_m_tr_FSQ-l_mu_tl_m_tr_BFSR 
gen wedge_norm =  (l_mu_tl_m_tr_FSQ-mean_q) - (l_mu_tl_m_tr_BFSR-mean_r)
gen l_mu_tl_m_tr_FSQ_norm = l_mu_tl_m_tr_FSQ-mean_q
gen l_mu_tl_m_tr_BFSR_norm  = l_mu_tl_m_tr_BFSR-mean_r
...
forval ind = 1/`max' { 
	//run bootstrap 
	bootstrap r(mean) r(sd) r(p50) r(p25) r(p75), reps(100): summarize l_mu_tl_m_tr_FSQ if indcount == `ind' ,d
	//save results
	...
	//run bootstrap 
	bootstrap r(mean) r(sd) r(p50) r(p25) r(p75), reps(100): summarize l_mu_tl_m_tr_BFSR if indcount == `ind' ,d
	//save results
	matrix results = r(table)
	preserve
		clear
...
putexcel set output/table4 , replace
