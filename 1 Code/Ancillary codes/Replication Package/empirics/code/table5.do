...
*** produce new table
statsby, by(naf2d_num) saving("temp\correlation.dta", replace): pwcorr 	   l_mu_tl_m_tr_FSQ      l_mu_tl_m_tr_BFSR  
statsby, by(naf2d_num) saving("temp\correlation_FD.dta", replace): pwcorr 	FD_l_mu_tl_m_tr_FSQ   FD_l_mu_tl_m_tr_BFSR
statsby, by(naf2d_num) saving("temp\correlation_spearman.dta", replace): spearman 	   l_mu_tl_m_tr_FSQ      l_mu_tl_m_tr_BFSR  
statsby, by(naf2d_num) saving("temp\correlation_spearman_FD.dta", replace): spearman   FD_l_mu_tl_m_tr_FSQ   FD_l_mu_tl_m_tr_BFSR 


*** pooled correlation 
pwcorr 	   l_mu_tl_m_tr_FSQ      	   l_mu_tl_m_tr_BFSR  
local store1 = r(rho)
pwcorr 	FD_l_mu_tl_m_tr_FSQ   		FD_l_mu_tl_m_tr_BFSR
local store2 = r(rho)
spearman      l_mu_tl_m_tr_FSQ         l_mu_tl_m_tr_BFSR 
local store3 = r(rho)
spearman   FD_l_mu_tl_m_tr_FSQ      FD_l_mu_tl_m_tr_BFSR 
local store4 = r(rho)
...
qui: su rho ,d 
matrix results =  [round(r(mean),0.01), round(r(sd),0.01), round(r(p50),0.01), round(r(p25),0.01), round(r(p75),0.01), round(`store1',0.001)] 
use temp\correlation_FD.dta , clear 
qui: su rho ,d 
matrix results =  results\ [round(r(mean),0.01), round(r(sd),0.01), round(r(p50),0.01), round(r(p25),0.01), round(r(p75),0.01), round(`store2',0.001)] 
use temp\correlation_spearman.dta , clear 
qui: su rho ,d 
matrix results = results\ [round(r(mean),0.01), round(r(sd),0.01), round(r(p50),0.01), round(r(p25),0.01), round(r(p75),0.01), round(`store3',0.001)] 
use temp\correlation_spearman_FD.dta , clear 
qui: su rho ,d 
matrix results =  results\[round(r(mean),0.01), round(r(sd),0.01), round(r(p50),0.01), round(r(p25),0.01), round(r(p75),0.01), round(`store4',0.001)] 
...
*** export
putexcel set output/table5, replace 
