...
* levels plot 
binscatter l_mu_tl_m_tr_FSQ l_mu_tl_m_tr_BFSR  , absorb(naf2d) graphregion(margin(0.5 0.5 0.5 4) lcolor(white)) xsize(5) ysize(4.5) aspectratio(1) reportreg nquantiles(45) lcolor(red) mcolors(blue)  msymbols(Oh) ytitle("Quantity-Based Markups") xtitle("Revenue-Based Markups") xlab(,grid glcolor(gs12))  ylab(,grid glcolor(gs12)) plotregion(lcolor(black)) legend(off) xlabel(-0.4(0.2)0.6,labsize(medlarge)) ylabel(0(0.2)0.8,labsize(medlarge))  // || twoway function l_mu_tl_m_tr_FSQ l_mu_tl_m_tr_BFSR //  xline(0,lstyle(grid) lwidth(*1.5) lcolor(black))    
graph export output\figur4_binscatter_p${code}.pdf, replace
//reghdfe l_mu_tl_m_tr_FSQ l_mu_tl_m_tr_BFSR  , absorb(naf2d) 


* fd plot 
binscatter FD_l_mu_tl_m_tr_FSQ FD_l_mu_tl_m_tr_BFSR  	, absorb(naf2d) graphregion(margin(0.5 0.5 0.5 4) lcolor(white)) xsize(5) ysize(4.5) aspectratio(1) reportreg nquantiles(45)  lcolor(red) mcolors(blue) msymbols(Oh) ytitle("Quantity-Based Markups ({&Delta})") xtitle("Revenue-Based Markups ({&Delta})") xlab(,grid glcolor(gs12))  ylab(,grid glcolor(gs12)) plotregion(lcolor(black)) legend(off) xlabel(-0.2(0.2)0.4,labsize(medlarge)) ylabel(-0.2(0.2)0.4,labsize(medlarge))  // || twoway function l_mu_tl_m_tr_FSQ l_mu_tl_m_tr_BFSR //  xline(0,lstyle(grid) lwidth(*1.5) lcolor(black))    
graph export output\figur4_binscatter_fd_p${code}.pdf, replace
