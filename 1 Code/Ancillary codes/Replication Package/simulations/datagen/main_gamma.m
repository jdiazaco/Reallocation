...
%%%% Exogenous Markup
Ex_mu = 0; % (0 for endogenous markup)
mu_std = 0.1; % standard deviation of markups
mu_m = 1.25; % mean of markups

%%%% Increasing murkup through productivity
Mkp_Trend = 0; % (0 for no trend in markup)

%%%% Produce graphs to check data
Graphs = 0; % (1 for producing graphs)

%%%% Export
Exporting = 1; % (0 for no export of data)
% Exporting = 0; % (0 for no export of data)

filename = 'firm_panel_gamma'; % calibration based on data
...
for Nk = [8]; for sigma_z = [0.2]; for rho_z=[0.6]; for sigma = [1.1]; for eta = [1.1]; for gamma = [0.6, 0.7, 0.8, 0.9, 1]; for alpha = [0.4]; for N_tot = [320000]; for sigma_wage=[sqrt(.06)]; for sigma_PsigmaY=[sqrt(0.19)]  % huge sample, also for Monte Carlo
...
opts1=  optimset('display',displaytype,'TolX',1e-20,'TolFun',1e-20,'MaxIter',1e4, 'MaxFunEvals',1e6);
