...
filename = 'firm_panel_alpha'; % calibration based on data
...
for Nk = [8]; for sigma_z = [0.2]; for rho_z=[0.6]; for sigma = [1.1]; for eta = [1.1]; for gamma = [0.8]; for alpha = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9]; for N_tot = [320000]; for sigma_wage=[sqrt(.06)]; for sigma_PsigmaY=[sqrt(0.19)]  % huge sample, also for Monte Carlo
...
for t=1:T
    ...
    if t==1 %faster but less stable
        ...
        if solver == "fsolve"
            x_CD= fsolve(fun_CD,[log(pki_monop);log(mcki_init)],options);
        elseif solver == "lsqnonlin" | solver == "iter"
%             if solver == "fsolve" | solver == "lsqnonlin" | solver == "iter"
            lb = [ones(Nk,1)*10^(-8); ones(Nk,1)*10^(-8)];
            ub = [ones(Nk,1); ones(Nk,1)*10^(8)];
            [x_CD,resnorm,residual,exitflag,output]= lsqnonlin(fun_CD,[log(pki_monop);log(mcki_init)],lb,ub,opts1);
            if exitflag<=0  disp(['CD initial values NOT CONVERGING: exitflag = ' num2str(exitflag)]); end
...
figNameEnd=['_', num2str(Comp),'_sigma',strrep(num2str(sigma),'.','d'),'_epsi',strrep(num2str(epsilonk),'.','d'),'_eta',strrep(num2str(eta),'.','d')];
