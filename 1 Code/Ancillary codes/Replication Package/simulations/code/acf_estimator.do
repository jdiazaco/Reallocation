...
local sale revenuefirm_obs

*variable for capital From 
...
if "$depvar" != "revenuefirm_obs" { 
	if "$depvar" == "revenuefirm_obs_1" { 
		local k = sqrt(1/`sigma_orig')	
	}
	if 	"$depvar" == "revenuefirm_obs_2" { 
		local k = sqrt(2/`sigma_orig')
	}
	
	if 	"$depvar" == "revenuefirm_obs_4" { 
		local k = sqrt(4/`sigma_orig')
	}
	
	if 	"$depvar" == "revenuefirm_obs_8" { 
		local k = sqrt(8/`sigma_orig')
	}
	
	if 	"$depvar" == "revenuefirm_obs_16" { 
		local k = sqrt(16/`sigma_orig')
	}
    gen eps_q_new = `k' * eps_q
	replace revenuefirm_obs = exp(r_defl + eps_q_new)
} 
...
if "`winsor'" == "1"{
	...
	gen k = log(`capital')  
	//below: the variable 'v' is either the variable input expenditure or the variable input quantity. By defining the variable here, 
	//the remainder of the code does not need to be updated when estimating markups using the quantity of the variable expenditure (variable v just gets different interpretation). 
	gen v = log(`varcost') 
	gen s = log(`sale')
}
...
qui: reg s v k o p  
matrix results = e(b)
matrix init_cd	= results[1,3], results[1,1], results[1,2]
reg s v k v2 k2 vk o p 
matrix results = e(b)
matrix init_tl	= results[1,8], results[1,1], results[1,2], results[1,3], results[1,4], results[1,5]
...
forval i = 0/`indcount' { //rmax is the number of industries in the specified number of industries (can view using the return list command after qui: sum ind 
	...
	*estimate regression with variable input and capital as well as all expansions/interactions and year f.e. 
	if "`order'" == "2" {
		qui: reg s v k v2 k2 vk i.year salessharefirm  
	}
	if "`order'" == "3" {
		qui: reg s v k v2 k2 vk v3 k3 v2k vk2 i.year salessharefirm  
	}
...
if "$depvar" == "revenuefirm_obs" { 
	if "$robustness" == "1" {
		save analysisdata/coefficients_byind_acf_$par, replace
	}
	if "$robustness" == "0" {
		save analysisdata/coefficients_byind_acf, replace
	}
}
else {
		save analysisdata/coefficients_byind_acf_$depvar, replace
}
