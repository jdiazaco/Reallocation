...
patent_data_upd<-read_parquet("C:/Users/Public/Documents/Big data Project/1) data/tm_patent/patent_record_level_final.parquet")
...
expand_siren_codes<-function(data, firm_var){
  ...
  # data[, siren_count:=lengths(strsplit(siren, ","))]
  # DT_expanded<-data[, .(firmid=unlist(strsplit(siren, ","))),
  #                   by=.(get(setdiff(names(data), "")))]
...
patent_data_upd<-patent_data_upd[, .(collection=paste(collection, collapse = ", ")), by=.(title, firmid, application_year)] 
...
# 4.3) Patent and trademark database creation-------------------

if(dummy){
  patent_and_tm = read_parquet('siren_level_patent_and_tm_final.parquet')
  test<-data.table(siren=unique(patent_and_tm$siren), number=seq_len(length(unique(patent_and_tm$siren))))
  test2<-data.table(number=unique(firm_data_select$firmid))
  test<-merge(test2, test, all.x=T, by="number")
  patent_and_tm<-merge(test, patent_and_tm,  by="siren", all.x = T)
  patent_and_tm$siren<-patent_and_tm$number
  patent_and_tm$number<-NULL
}else{
...
# # 4.4) LinkedIn data -------------------
# linkedin_data<-read_parquet("C:/Users/Public/Documents/Big data Project/1) data/15_french_affiliated_firm_roles_collapsed_clean.parquet")
# setorder(linkedin_data, firmid, year)
# saveRDS(linkedin_data, "linkedin_data.RDS")




# 4.5) Patent trend graph -------------------
...
# Clean the trademark_status field and assign group
tm_data[, trademark_status_clean := str_trim(str_replace_all(trademark_status, "&apos;", "'"))]

# Define the mapping using grouped conditions
tm_data[, status_group := fcase(
  # Applications
  trademark_status_clean %in% c("Demande déposée", "Application filed",
                                "Demande publiée", "Application published",
                                "Application under examination",
                                "Demande irrecevable après publication"), "Application",
  
  # Registrations
  trademark_status_clean %in% c("Marque enregistrée", "Registered",
                                "Registration pending"), "Registration",
  
  # Renovations
  trademark_status_clean %in% c("Marque renouvelée", "Renouvellement demandé"), "Renovation",
  
  
  # Inactive / expired / rejected
  trademark_status_clean %in% c("Marque expirée", "Registration expired",
                                ...
                                "Marque ayant fait l'objet d'un retrait total"), "Inactive",
...
table(tm_data_2022$status_group)

tm_data<- setDT(tm_data)
tm_data<-tm_data[, .(n=.N), by=.(application_year)]
ggplot(tm_data, aes(x=application_year, y=n)) + 
  geom_point() + 
  geom_line()
      ...
      for(i in innovation_vars){
        ...
        test <- product_data[, {
          ...
          .(
            ...
            rev_growth = mean(rev_growth, na.rm=TRUE)
          )
        }, by = .(firmid, get(j))]
        ...
        lag_share_core <- if (cpa_or_pf == "cpa") "lag_share_core_pf_6" else "lag_share_core_pf_10"
        lag_share_runup <- if (cpa_or_pf == "cpa") "lag_share_runup_pf_6" else "lag_share_runup_pf_10"
        ...
        if(i=="all"){
          ...
          ggplot(restult, aes(x=bin, y=weighted_ex_rate)) + 
            ...
            ggtitle(paste0("Product Exit Rates by Firm-", j, " ", distrib_text),
