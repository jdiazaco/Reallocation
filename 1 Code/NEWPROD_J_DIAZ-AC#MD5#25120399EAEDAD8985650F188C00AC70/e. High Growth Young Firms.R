...
folder_name<-""
  output_dir<-paste0(output_dir, "2025/Export 07.02/High growth young firms_adj_final/")
...
for(subset in c("young", "mature")){
  ...
  if(subset=="young"){
    ...
    xs<-list(c("net_product_creat_window*patent_window*size","net_product_creat_window*tm_window*size", "net_product_destr_window*size"))
  }else{
    if(subset=="mature"){
  ...
  for(growth_var in growth_vars){
    ...
    for(y in ys){
      ...
      for (digit in c("all")) {
        ...
        continuing_firms[, top_growth:=fifelse(quantile(get(paste0(growth_var, "_growth")), probs=1-threshold_growth, na.rm=T),1,0), by=.(year)]
        ...
        make_summary_stats(product_summary_temp, 
                           c("firm_age", "empl_bar", "nq_bar", "empl_growth", "nq_growth", "number_of_products", "prod_creat", "prod_destr", "patent"),
                           "top_growth",
                           paste0("summary_stats_top_growth_", paste0(growth_var, "_growth")))
          ...
          for(f in fix_eff){
            for(i in c("variables")){
              for(fil in get(i)){
                ...
                # Define table titles
                title<-toTitleCase(paste0("Regressions of ", gsub("_", " ", y), " on product and innovation variables "))
                ...
                # Export results
                modelsummary(regressions_per_filter_fe, output=paste0(output_dir, "regressions_", y, "_", fil, "_", f, ".tex"), 
                             label=paste0("regressions_", y, "_", fil, "_", f),
                             stars = T, 
