...
# firm_yr_lvl_dta <- firm_yr_lvl_dta[abs(empl_growth)!=2 & abs(nq_growth)!=2 &  abs(capital_growth)!=2 & empl>=20, c("firmid", "year", "NACE_BR", "empl")]
firm_yr_lvl_dta <- firm_yr_lvl_dta[empl>=20, c("firmid", "year", "NACE_BR", "empl")]; gc()


start <- 1994
end<-2022
firm_yr_lvl_dta <- growth_creator(firm_yr_lvl_dta, "empl", 1); gc()
firm_yr_lvl_dta <- firm_yr_lvl_dta[abs(empl_growth)!=2 & empl>=20]; gc()
firm_yr_lvl_dta <- firm_yr_lvl_dta[year<economic_death_year]; gc()
# firm_yr_lvl_dta <- fread('1_temp.csv')


# ## carry out the decompositions  
# vars = c('rev', 'entrance', 'exit', rep('empl', 4))
# data_frames = c(rep('agg_product_data',3), 'firm_yr_lvl_dta', 'firm_yr_lvl_dta[status=="survived"]', 'firm_yr_lvl_dta[empl<20]', 'firm_yr_lvl_dta[empl>=20]')
...
dyn<-"reallocation"
    ...
    if(vars[i] != "empl" | data_frames[i]=="firm_yr_lvl_dta"){
      firm_data = get(data_frames[i])
    }else{
      firm_data = eval(parse(data_frames[i]))
    }
    ...
    # firm_data <- firm_data[year<=economic_death_year]; gc()
    ...
    ggplot(decompositions[year %in% 2004:2015], aes(x=year, y=agg_ma3)) + geom_line()+
      scale_y_continuous(labels = scales::label_percent(scale = 100), limits = c(0.06, 0.10)) +
      ...
      labs(title =  "Labor Reallocation Rate - 3-year Moving Average (2004-2015)", subtitle = "Continuing firms with >=20 employees")+ guides(size = 'none')
    ggsave(paste0(output_dir, "emp_reallocation_continuers_20e_3ma_2004_2015.png"), height = 6, width = 7)
    ...
    theme_classic() +theme(legend.position = 'bottom', axis.title.x = element_blank(),
                           axis.ticks.x = element_blank(), axis.title.y = element_blank(),
                           axis.line.x = element_blank(), legend.title = element_blank()) + 
      labs(title =  "Labor Reallocation Rate - 3-year Moving Average", subtitle = "Continuing firms with >=20 employees")+ guides(size = 'none')
    ggsave(paste0(output_dir, "emp_reallocation_continuers_20e_3ma.png"), height = 6, width = 7)
    ...
    if (vars[i]=='entrance'){
      ...
      decompositions_long = melt(decompositions, id.vars = 'year', measure.vars = sub_groups) %>%
        rename(sub_group = variable)
      saveRDS(decompositions_long, paste0(output_dir, 'firm_level_',vars[i],'_', dyn, '_continuers_20e_', remove_special_chars(data_frames[i]), '.rds'))
    }
...
#   }))
    var<-"empl"
  realloc_results<-decompositions_long
  ...
  for (i in seq_along(vars)){
    ...
    assign(paste0(vars[i],'_delta_decomp'), ggplot(data[sub_group %in% c('agg', 'within', 'between', 'cross','entry/exit', 'agg_ma3')],
             ...
             theme_classic() +theme(legend.position = 'bottom', axis.title.x = element_blank(),
                                    axis.ticks.x = element_blank(), axis.title.y = element_blank(),
                                    axis.line.x = element_blank(), legend.title = element_blank()) + 
             labs(title =  TeX(titles[i]), subtitle = "Continuing firms with >=20 employees")+ guides(size = 'none') +
             geom_hline(yintercept = 0, linetype = 'dashed') +
             scale_color_hue(labels = c('agg' = 'Reallocation Rate', 'agg_ma3' = '3-year MA',       'Decompostion:',
                                        'within'= 'within', 'between' = 'between', 
                                        'cross' = 'cross', 'entry/exit' = 'entry/exit'))
    )
    ggsave(paste0(output_dir, dyn, '_', vars[i],'_continuers_20e_delta_decomp.png'), get(paste0(vars[i],'_delta_decomp')), height=5, width=9)
  }
}
