...
# sbs_br_combined[, size:=case_when(empl<1 ~ "micro",
#                                   empl>=1 & empl<50 ~ "small",
#                                   empl>=50 & empl < 250 ~ "medium",
#                                   empl>=250  ~ "large")]
# 
# data_share<-sbs_br_combined[, .N, by=.(firm_birth_year, size)]
# data_share[,total:=sum(N), by=firm_birth_year]
# data_share[, share:=N/total]
# 
# hist(as.numeric(sbs_br_combined$firm_birth_year), bins=125)
