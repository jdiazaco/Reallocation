...
output_dir<-paste0(output_dir, "Product reallocation and firm dynamics/Export 15.11/disaggregated entry and exit on growth/")
...
core_switch_product<-function(data, n_digits){
  ...
  data[[pf]]<-substr(data$cpa, 1, n_digits) # Create the category variable (sector, industry, cpa, pcc8, prodfra)
  if(n_digits==1){
...
product_data<-product_data[, `:=`(new_6=first_introduction & cpa==lag_core_pf_6)]
product_data<-product_data[, `:=`(new_4=first_introduction & NACE==lag_core_pf_4 & new_6!=T)]
product_data<-product_data[, `:=`(new_2=first_introduction & NACE_2d_pf==lag_core_pf_2 & new_4!=T & new_6!=T)]
product_data<-product_data[, `:=`(new_1=first_introduction & substr(DEFind,1,1)==lag_core_pf_1 & new_6!=T & new_4!=T & new_2!=T)]
product_data<-product_data[, `:=`(new_0=first_introduction & substr(DEFind,1,1)!=lag_core_pf_1 & new_6!=T & new_4!=T & new_2!=T & new_1!=T )]

product_data<-product_data[, `:=`(exit_6=discontinued & cpa==lag_core_pf_6)]
product_data<-product_data[, `:=`(exit_4=discontinued & NACE==lag_core_pf_4 & exit_6!=T)]
product_data<-product_data[, `:=`(exit_2=discontinued & NACE_2d_pf==lag_core_pf_2 & exit_6!=T & exit_4!=T)]
product_data<-product_data[, `:=`(exit_1=discontinued & substr(DEFind,1,1)==lag_core_pf_1 & exit_6!=T & exit_4!=T & exit_2!=T)]
product_data<-product_data[, `:=`(exit_0=discontinued & substr(DEFind,1,1)!=lag_core_pf_1 & exit_6!=T & exit_4!=T & exit_2!=T & exit_1!=T)]
...
write_rds(product_data, paste0("product_level_growth_", filter_indicator,  "_new_core_analysis_cpa.RDS"))
write_rds(product_core, paste0("product_core_cpa.RDS"))
...
product_data<-readRDS(paste0("product_level_growth_", filter_indicator,  "_new_core_analysis_cpa.RDS"))
product_core<-readRDS(paste0("product_core_cpa.RDS"))

# Define working variables
vars<-c("first_introduction", "new_6", "new_4", "new_2", "new_1", "new_0", 
        # "switch_pf_10", "switch_pf_8", "switch_pf_6", "switch_pf_4", "switch_pf_2",
        "exit_6", "exit_4", "exit_2", "exit_1", "exit_0", 
...
saveRDS(product_summary, paste0("product_creation_destruction_eppe_cpa.RDS"))

# 2.2) Lag and lead product entry and exit variables -----

product_summary<- readRDS(paste0("product_creation_destruction_eppe_cpa.RDS"))
...
saveRDS(product_summary, paste0("product_creation_destruction_cpa.RDS"))
...
product_data<-readRDS(paste0("product_level_growth_", filter_indicator,  "_new_core_analysis_cpa.RDS"))
product_summary<-readRDS(paste0("product_creation_destruction_cpa.RDS"))
product_core<-readRDS(paste0("product_core_cpa.RDS"))
...
labels_codes<-c("No Common Root", "1 Letter", "2 Digits", "4 Digits", "6 Digits")
  ...
  for(k in samples){
    ...
    for(j in category_vars){
      ...
      for(i in innovation_vars){
        ...
        test<-product_data[, .(n_new_products=n_distinct(cpa[get(new_var) & year!=first_year & rev>rev_limit], na.rm = T),
                               new_codes=paste(unique(cpa[get(new_var) & year!=first_year & rev>rev_limit], na.rm = T), collapse=", "),
                               n_exit_products=n_distinct(cpa[get(exit_var) & year!=last_year], na.rm = T),
                               share_exit_product=mean(within_firm_rev_share[get(exit_var) & year!=last_year], na.rm = T),
                               lag_share_core_pf_6=mean(lag_share_core_pf_6[get(exit_var) & year!=last_year], na.rm = T),
                               lag_share_runup_pf_6=mean(lag_share_runup_pf_6[get(exit_var) & year!=last_year], na.rm = T),
                               exit_codes=paste(unique(cpa[get(exit_var) & year!=last_year], na.rm = T), collapse=", "),
                               n_products=n_distinct(cpa),
                               codes=paste(unique(cpa, na.rm = T), collapse=", "),
                               rev=mean(rev, na.rm=T),
                               age=mean(firm_age, na.rm = T)), by=.(firmid, (get(j)))]
        ...
        variables_to_weight <- c("in_rate", "ex_rate", "share_exit_product", "lag_share_core_pf_6", "lag_share_runup_pf_6")
      ...
      ggplot(all_results, aes(x=bin, y=weighted_in_rate, fill=innovation_var)) + 
        ...
        ylim(0, max(0.4, all_results$weighted_in_rate))+
      ...
      ggplot(all_results, aes(x=bin, y=weighted_ex_rate, fill=innovation_var)) + 
        geom_bar(stat = "identity") + 
        scale_fill_discrete(labels=labels_codes, name="Common Root with Core")+
        ylim(0, max(0.4, all_results$weighted_ex_rate))+
      ...
      ggplot(all_results_wide, aes(x = bin)) + 
        geom_line(aes(y = weighted_lag_share_runup_pf_6_6, color = "Runner-up (w)"), size = 1) +
        geom_line(aes(y = weighted_lag_share_core_pf_6_6, color = "Core (w)"), size = 1) +
        geom_line(aes(y = unweighted_lag_share_runup_pf_6_6, color = "Runner-up (uw)"), size = 1) +
        geom_line(aes(y = unweighted_lag_share_core_pf_6_6, color = "Core (uw)"), size = 1) +
        labs(
          x = "Size Percentile",
...
for (level in levels) {
  ...
  formula_disag_reallocation<-"top_growth ~ new_0 +  new_1 + new_2 + new_4 + new_6 + exit_0 + exit_1 + exit_2 + exit_4 + exit_6 + patenting"
