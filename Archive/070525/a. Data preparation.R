...
- Import CIS data
- Add Alex's linkedin compensation data
- Adjust firmid in all datasets to be 9 digits long (fill left side with 0s when number of characters is lower than 9)
- Create firm_birth_year, a variable with the year the firm was created according to BR with firm_birth_year_creator.R
- Add turnover, raw_materials, labor_cost, firm_birth_year and capital growth and reallocation measures in the biennial version of sbs/br
...
- Fix missing NACE codes by assuming that the industry of the firm when the NACE code is missing is the same as the previous available year

Author: Alex Magnuson
Edits: Juli?n D?az-Acosta
...
hist(sbs_br_combined$firm_birth_year, breaks = 121)

sbs_br_combined<-deflate(sbs_br_combined, "NACE_BR", c("nq", "capital", "turnover", "raw_materials", "labor_cost"), 2009)
...
NACE_data<-readRDS('firm_NACE_BR.rds') %>% select(firmid, year, NACE_BR)

cis_years<-c("2010", "2012", "2014", "2016", "2018")

## import BR data
cis_data = rbindlist(lapply(cis_years,function(yr){
  print(yr)
  ## import BR data
  cis_path = paste0(raw_dir,"cis/cis",yr,".csv" )
  cis_data_temp = fread(cis_path)
  cis_data_temp[, `:=`(firmid = as.character(firmid))]
}), fill = T)

cis_data$firmid<-str_pad(cis_data$firmid, width = 9, side="left", pad="0")

cis_data<-merge(cis_data, NACE_data, by=c("firmid", "year"), all.x = T)

cis_data<-deflate(cis_data, "NACE_BR", c("rrdinx", "rrdexx", "rmacx", "rtot"), 2009)

saveRDS(cis_data, 'cis_data.RDS')
...
## define firm birth/death as first/last time 
## with positive employees in the data 
sbs_data[, `:=`(birth_year = min(year),
                death_year = max(year)), by = firmid]

# Juli?n: Create share costs of factors and tfp
share_capital_costs<-0.1

sbs_data<-sbs_data[, `:=`(sum_costs=(labor_cost + (capital*share_capital_costs) + raw_materials))]
sbs_data<-sbs_data[, `:=`(t_l=labor_cost/sum_costs,
...
sbs_data<-sbs_data[, log_tfp:=(log(nq)-(t_l*log(labor_cost))-(t_k*log(capital))-(t_m*log(raw_materials)))]
sbs_data[, log_tfp:=ifelse(is.nan(log_tfp) | is.infinite(log_tfp), NA_real_, log_tfp)]
sbs_data[, `:=`(tfp=exp(log_tfp))]


## generate lagged variables Juli?n: add capital  
normal_cols = c('nq','empl', "tfp", 'capital', "turnover", "raw_materials", "labor_cost", "firm_birth_year")
lag_cols = paste0(normal_cols,'_l')

sbs_data_l = sbs_data[year<end] %>% mutate(year = year + 1) %>% select(-NACE_BR) %>% select (firmid, year, birth_year, death_year, normal_cols)
...
setorder(sbs_data, firmid, year)
sbs_data[, `:=`(NACE_BR=zoo::na.locf(NACE_BR, na.rm=F)), by=firmid]
...
sbs_data = sbs_data %>% select(c("firmid", "NACE_BR" ,'year', "birth_year" , "death_year" ,'age','young',"born", "died", "status", "industry",
...
sbs_data[, size:=case_when(empl_bar<1 ~ "micro",
                           empl_bar>=1 & empl_bar<50 ~ "small",
                           empl_bar>=50 & empl_bar < 250 ~ "medium",
                           empl_bar>=250  ~ "large")]

# Juli?n: Drop micro firms
# sbs_data <- sbs_data[size != "micro" | is.na(size)]
# sbs_data <- sbs_data[size != "micro"]
table(sbs_data$size, useNA = "always")

# Juli?n Create age and young indicators
sbs_data <- sbs_data %>% mutate(firm_birth_year=ifelse(firm_birth_year==0, NA, firm_birth_year),
                                birth_year=fifelse(birth_year==start, 
                                                   fifelse(firm_birth_year<start, firm_birth_year, start), birth_year),
                                firm_age=year-birth_year,
...
saveRDS(NACE_BR_data, 'NACE_BR_data.rds')
...
#4pf) clean product level data from Prodcom  ----------------------------------------------
...
#Aggregate at the 6 digits (cpa) level
...
# Create alternative NACE information: Take the first 4 digits of the cpa code
...
product_data<-product_data %>% group_by(firmid) %>% mutate( first_year=min(year), last_year=max(year) )
product_data <- product_data %>% group_by(firmid, cpa) %>% mutate(forward_year=dplyr::lead(year), lag_year=dplyr::lag(year))
#Juli?n: Create prodcom coverage gap variables
product_data <- product_data %>% group_by(firmid, cpa) %>% 
  mutate(gap=ifelse(forward_year==(year+1) & lag_year==(year-1), 0, ifelse(forward_year==year | lag_year==year, 0, 1)))
product_data <- product_data %>% mutate(gap=ifelse(is.na(gap),0,gap))

product_data <- product_data %>% select(firmid, cpa, year, first_year, last_year, gap, forward_year, lag_year, everything())
product_data<-as.data.table(product_data)
product_data[, rev_bar := .5*(rev + rev_l)]
#Juli?n: delete absolute values to have actual revenue growth (not reallocation)
product_data[, rev_growth := ifelse(rev_bar != 0, (rev - rev_l)/rev_bar, 0)]
...
product_data[, active_year := ifelse(active, year, NA)]
product_data[, `:=`(first_introduction = year == min(active_year, na.rm = T),
                    discontinued = year>max(active_year, na.rm = T) & !active), by= .(firmid, cpa)]
product_data[, `:=`(reintroduced = !first_introduction & active & !active_l,
                    paused= !discontinued & active_l & !active,
                    incumbent = active_l & active)]

## export the data 
product_data = product_data %>% arrange(firmid, cpa, year) %>% select(columns, everything())
...
raw_ep_path<-"//casd.fr/casdfs/Projets/NEWPROD/Data/Statistique annuelle d'entreprise_Contours des entreprises profil?es_"
